...
class MessageOrchestrator:
    ...
    async def agentic_text(
        ...
        # INSTANT ACK — fire the placeholder reply BEFORE rate-limit and any heavy
        # setup so the user sees a "thinking…" message immediately, regardless of
        # what else is in flight. progress_msg is then either edited (rate-limit
        # denial, errors, "Working..." for streaming) or deleted right before the
        # real response is posted.
        ...
        if rate_limiter:
            ...
            if not allowed:
                await progress_msg.edit_text(f"⏱️ {limit_message}")
                return

        # MOCK_RESPONSES.json (per-bot cwd) drives THREE deterministic, no-LLM behaviors:
        #   1. patterns[]  → short-circuit: edit progress_msg with canned response, no Claude call
        #   2. preflight[] → send extra warning message, then CONTINUE to Claude
        #   3. limits{}    → max_seconds + max_tool_calls watchdog around the Claude call below
        # First matching regex per section wins. File is reread on every message,
        # so patterns/limits are tunable without bot restart.
        mock_cfg: Dict[str, Any] = {}
        mock_path = Path.cwd() / "MOCK_RESPONSES.json"
        if mock_path.exists():
            try:
                import json as _json
                mock_cfg = _json.loads(mock_path.read_text(encoding="utf-8"))
            except Exception as _exc:  # noqa: BLE001 — never break the bot
                logger.warning("MOCK_RESPONSES.json read failed", error=str(_exc))

        # (1) Short-circuit response patterns
        for _entry in mock_cfg.get("patterns", []):
            _pat = _entry.get("regex", "")
            _resp = _entry.get("response", "")
            if _pat and _resp and re.search(_pat, message_text):
                await progress_msg.edit_text(_resp)
                logger.info(
                    "Mock response sent (bypassed Claude)",
                    pattern=_pat,
                    user_id=user_id,
                    chat_id=update.effective_chat.id,
                )
        ...
        # (2) Pre-flight warnings — emit extra message, continue to Claude.
        for _entry in mock_cfg.get("preflight", []):
            _pat = _entry.get("regex", "")
            _resp = _entry.get("response", "")
            if _pat and _resp and re.search(_pat, message_text):
                try:
                    await update.effective_chat.send_message(_resp)
                    logger.info("Preflight warning sent", pattern=_pat, user_id=user_id)
                except Exception as _exc:  # noqa: BLE001
                    logger.warning("Preflight send failed", error=str(_exc))
                break

        # (3) Limits — read once, applied by the watchdog wrapping run_command below.
        _limits = mock_cfg.get("limits", {}) if isinstance(mock_cfg, dict) else {}
        max_seconds = _limits.get("max_seconds", 300)
        max_tool_calls = _limits.get("max_tool_calls", 15)
        timeout_response = _limits.get("timeout_response", "🧊 This is using too much snow.")
        tool_limit_response = _limits.get("tool_limit_response", "🧊 This is using too much snow.")

        chat = update.message.chat
        await chat.send_action("typing")

        verbose_level = self._get_verbose_level(context)
        # progress_msg was created above as instant ACK; update its text to "Working..."
        # so the streaming updates and verbose progress have a familiar starting point.
        try:
            await progress_msg.edit_text("Working...")
        except Exception:
            pass  # cosmetic only — already showing "thinking…"
        ...
        try:
            # Watchdog: enforces max_seconds (wall-clock) and max_tool_calls
            # (count of {"kind":"tool"} entries in tool_log) by cancelling
            # the Claude SDK task. Set abort_reason; outer block edits
            # progress_msg with the configured response and returns.
            async def _limits_watchdog() -> None:
                nonlocal abort_reason
                while True:
                    await asyncio.sleep(2)
                    _count = sum(1 for e in tool_log if e.get("kind") == "tool")
                    if _count > max_tool_calls:
                        abort_reason = "tool_limit"
                        return

            _run_task = asyncio.create_task(claude_integration.run_command(
                prompt=message_text,
            ...
            _watch_task = asyncio.create_task(_limits_watchdog())

            try:
                _done, _pending = await asyncio.wait(
                    {_run_task, _watch_task},
                    timeout=max_seconds,
            ...
            if not _done:
                # asyncio.wait hit the timeout — wall-clock cap reached
                ...
                try:
                    ...
                    pass
            elif _run_task in _done:
                # Normal completion — .result() re-raises any exception
                # which the outer except handlers catch.
                claude_response = _run_task.result()
            else:
                # Watchdog finished first → tool-count abort
                _run_task.cancel()
                try:
                    await _run_task
                except (asyncio.CancelledError, Exception):
            ...
            if abort_reason:
                heartbeat.cancel()
                _resp = timeout_response if abort_reason == "timeout" else tool_limit_response
                try:
                    await progress_msg.edit_text(_resp)
                except Exception:
                    pass
                logger.info(
                    "Claude call aborted by no-llm limits",
                    reason=abort_reason,
